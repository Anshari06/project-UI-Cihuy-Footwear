...
@section('admin-title', 'Edit Artikel')
    ...
    <form action="{{ route('admin.artikel.update', $artikel->id) }}" method="POST">
        ...
        <div class="row g-4">
            <div class="col-12">
                ...
                <input type="text" name="title" value="{{ $artikel->title }}" class="form-control" required style="border: 1.5px solid #e6e0da; border-radius: 8px; padding: 10px 14px; font-size: 14px;">
            </div>
            <div class="col-md-6">
                <label class="form-label" style="font-size: 13px; font-weight: 600; color: #2b2b2b;">Author</label>
                <input type="text" name="author" value="{{ $artikel->author }}" class="form-control" placeholder="Nama author" style="border: 1.5px solid #e6e0da; border-radius: 8px; padding: 10px 14px; font-size: 14px;">
            </div>
            <div class="col-md-6">
                ...
                <select name="category" class="form-select" style="border: 1.5px solid #e6e0da; border-radius: 8px; padding: 10px 14px; font-size: 14px;">
                    <option value="all" {{ $artikel->category === 'all' ? 'selected' : '' }}>Terbaru</option>
                    <option value="trending" {{ $artikel->category === 'trending' ? 'selected' : '' }}>Trending</option>
                </select>
            </div>
            <div class="col-md-6">
                <label class="form-label" style="font-size: 13px; font-weight: 600; color: #2b2b2b;">URL Gambar</label>
                <input type="text" name="image" value="{{ $artikel->image }}" class="form-control" placeholder="https://images.unsplash.com/..." style="border: 1.5px solid #e6e0da; border-radius: 8px; padding: 10px 14px; font-size: 14px;">
            </div>
            <div class="col-12">
                <label class="form-label" style="font-size: 13px; font-weight: 600; color: #2b2b2b;">Konten</label>
                <textarea name="content" class="form-control" rows="8" required style="border: 1.5px solid #e6e0da; border-radius: 8px; padding: 10px 14px; font-size: 14px; resize: vertical;">{{ $artikel->content }}</textarea>
            </div>
        </div>
        <div class="mt-4">
            <button type="submit" class="btn" style="background: #545350; color: #fff; border-radius: 8px; padding: 10px 28px; font-weight: 600; font-size: 14px;">
                <i class="bi bi-check-lg me-1"></i> Update Artikel
            </button>
        </div>
    </form>
</div>
